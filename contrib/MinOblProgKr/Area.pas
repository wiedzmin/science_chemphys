...
begin
  ...
  for i := 0 to k - 1 do begin
    ...
    c1 := false;
    c2 := false;
    fname := form1.atom + form1.edit1.Text +
      form1.edit2.Text + 'pointpath' + inttostr(i) + {inttostr(form1.number) + }'.txt';
    assignfile(tf, fname);
    if fileexists(fname) then begin
      append(tf);
    end else begin
    ...
    repeat
      ...
      if (ab = 1) and (j = 0) and (pointt.rekomb = false) then begin
        koef.bet := 1; koef.tet := 1; koef.fi := 1; koef1 := 1;
        ab := -1; j := 0; c1 := false; c2 := false;
        pointt := sympl1[i];
        point0 := pointt;
      end;
      inc(j);
    ...
    until ((ab = 1) and ((pointt.rekomb = false) or ((pointt.tet = pointprev.tet) and (pointt.fi = pointprev.fi) and
