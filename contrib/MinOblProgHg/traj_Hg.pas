...
begin
  ...
  if recomb then begin
    ...
    ZV := round(EVIB/ZG - 0.5); // колеб. квант. число
    ZF := 1/(2 * MU2 * sqr(SQ));
    ZJ := round(SQRT(EROT/ZF + 0.25) - 0.5); // вращ. квант. число
    NLO := 1;
...
begin
  ...
  TCOL := SQRT(MU2/2) / C * (1/2 / SQRT(C) *
                               Ln(2 * SQRT(C * (C * sqr(RINI) + RINI)) +
...
begin
  ...
  with inifile do begin
    a := readfloat('Atom', 'a',1 27.5);
    s := readfloat('Atom', 's',0 .7073);
    c := readfloat('Atom', 'c',8 7.36);
    ...
    alf2 := readfloat('Atom', 'alf2 ',16.48);
    alf3 := readfloat('Atom', 'alf3 ',32.46);
