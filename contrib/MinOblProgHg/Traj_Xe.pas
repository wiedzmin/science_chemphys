...
begin
  ...
  repeat
    ...
    IF(R[1] > RINI) AND (R[2] < RZ) AND (R[3] > RINI) THEN begin
      ...
      IF (ER < 0) then begin recomb := true; break; end;
    end;
...
var
  ...
  begin
    result := A2*EXP(-R2/S2) - 1/R2 - (ALF2 + ALF3) / 2 / power(R2, 4) - C2/power(R2, 6) +
      A1*EXP(-R1/S1) + A3*EXP(-R3/S3) - C1/power(R1, 6) - C3/power(R3, 6) - ALF1/2 *
...
begin
  ...
  VRF2 := (-A2/S2 * EXP(-R[2]/S2) + 1/sqr(R[2]) + 2*(ALF2 + ALF3)/power(R[2], 5) + 6*C2/power(R[2], 7) -
             ALF1*R[2]/power(R[1],3)/power(R[3],3)*sqr(1 - (ALF2 + ALF3)/power(R[2],3)) -
             ALF1*(1/power(R[1],4) + 1/power(R[3],4) + (sqr(R[2]) - sqr(R[1]) - sqr(R[3]))/power(R[1], 3) /
                     power(R[3], 3))*(1-(ALF2 + ALF3)/power(R[2],3))*3*(ALF2 + ALF3)/power(R[2],4))/R[2];
...
begin
  ...
  TCOL := SQRT(MU2 / 2) / C*(1 / 2 / SQRT(C) * Ln(2 * SQRT(C * (C * sqr(RINI) + RINI)) + 2*C*RINI + 1) -
