...
begin
  ...
  repeat
    ...
    for L := 1 to 6 do begin
      P[L] := P[L] + HT/1440*(4277*ZA[L, 6] - 7923*ZA[L, 5] +
                                9982*ZA[L, 4] - 7298*ZA[L, 3] + 2877*ZA[L, 2] - 475*ZA[L, 1]);
      Q[L] := Q[L] + HT/1440*(4277*ZA[L+6, 6] - 7923*ZA[L+6, 5] +
                                9982*ZA[L+6,4]-7298*ZA[L+6, 3] + 2877*ZA[L+6, 2] - 475*ZA[L+6, 1]);
    end;
    for L := 1 to 12 do
      for K := 1 to 5 do
        ZA[L, K] := ZA[L, K+1];
...
var
  ...
  begin
    result := A2 * EXP(-R2 / S2) - 1/R2 - (ALF2 + ALF3) / 2
      / power(R2, 4) - C2/power(R2, 6) + A1*EXP(-R1 / S1) +
      A3*EXP(-R3/S3) - C1/power(R1, 6) - C3/power(R3, 6) -
      ALF1/2 * (1 / power(R1, 4) + 1 / power(R3, 4) + (sqr(R2) - sqr(R1) - sqr(R3))/power(R1, 3)/power(R3,3)) *
...
begin
  ...
  ESYS := F1(R[1], R[2], R[3]);
  VRF1 := (-A1/S1*EXP(-R[1]/S1) + 6*C1/power(R[1], 7) +
             ALF1/2*sqr(1 - (ALF2 + ALF3)/power(R[2], 3)) *
             (4/power(R[1], 5) + (3*sqr(R[2]) - sqr(R[1]) - 3*sqr(R[3]))/power(R[1], 4)/power(R[3], 3)))/R[1];
  VRF2 := (-A2/S2*EXP(-R[2]/S2) + 1/sqr(R[2]) + 2*(ALF2 + ALF3)/power(R[2], 5) +
             6*C2/power(R[2], 7) - ALF1*R[2]/power(R[1], 3)/power(R[3], 3) * sqr(1 - (ALF2 + ALF3)/power(R[2], 3)) -
             ALF1 * (1/power(R[1], 4) + 1/power(R[3], 4) + (sqr(R[2])-sqr(R[1])-sqr(R[3]))/power(R[1],3)/power(R[3],3)) *
...
begin
  ...
  TCOL := SQRT(MU2 / 2) / C *
    (1/2 / SQRT(C) * Ln(2 * SQRT(C * (C * sqr(RINI) + RINI)) + 2*C*RINI + 1) -
       SQRT(C * sqr(RINI) + RINI)); // dlog = ln ???
  ATET := t.TET / 180. * ZPI;
  AFI := t.FI / 180. * ZPI;
