...
begin
  if Application.MessageBox('Возобновить расчет по сетке, который был прерван?','Контроль', MB_OKCANCEL) <> IDOK then exit;
  timer1.Enabled := true;
  continue2 := true;
  continue;
end;
...
begin
  ...
  Label6.Caption := 'Осталось: ' + timetostr((osttime)) + ' с.' + '  Осталось рассчитать: ' + floattostr(oststep) + ' шагов';
end;
