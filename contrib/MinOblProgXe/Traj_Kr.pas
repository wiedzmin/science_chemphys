...
{$R *.dfm}
uses
  mmsystem, inifiles, minimum, area, traj_Hg, traj_Xe, setka;

var
  inifile: tinifile;

procedure tform5.Integr;
var
...
begin
  ...
  if recomb then begin
    ...
    t.emolec := (EVIB + EROT)/3.67502E-2;
    t.eout := ((((sqr(rm[kt - 1][11] * (1 + M1/(M2 + M3)))) * MU) / 2)) / 3.67502E-2 - t.erel;
    if (t.emolec < 2) or (t.emolec > 99) then
      t.molec := false
    else
      t.molec := true;
  end;
...
procedure tform5.fun;
var
  vrf1, vrf2, vrf3: real;
  function f1(r1, r2, r3: real): real;
  begin
    result := A2*EXP(-R2/S2) - 1/R2 - (ALF2 + ALF3)/ 2 / power(R2, 4) - C2/power(R2, 6) +
              A1*EXP(-R1/S1) + A3*EXP(-R3/S3) - C1/power(R1, 6) - C3/power(R3, 6) -
              ALF1/2 * (1/power(R1, 4) + 1 / power(R3, 4) + (sqr(R2) - sqr(R1) - sqr(R3)) /
                          power(R1, 3) / power(R3, 3)) * sqr(1 - (ALF2 + ALF3)/power(R2,3));
  end;
begin
  ...
  ESYS := F1(R[1],R[2],R[3]);
  VRF1 := (-A1/S1*EXP(-R[1]/S1)+6*C1/power(R[1],7)+ALF1/2*sqr(1-(ALF2+ALF3)/power(R[2],3))*(4/power(R[1],5)+(3*sqr(R[2])-sqr(R[1])-3*sqr(R[3]))/power(R[1],4)/power(R[3],3)))/R[1];
  VRF2 := (-A2/S2*EXP(-R[2]/S2)+1/sqr(R[2])+2*(ALF2+ALF3)/power(R[2],5)+6*C2/power(R[2],7)-ALF1*R[2]/power(R[1],3)/power(R[3],3)*sqr(1-(ALF2+ALF3)/power(R[2],3))-ALF1*(1/power(R[1],4)+1/power(R[3],4)+(sqr(R[2])-sqr(R[1])-sqr(R[3]))/power(R[1],3)/power(R[3],3))*(1-(ALF2+ALF3)/power(R[2],3))*3*(ALF2+ALF3)/power(R[2],4))/R[2];
  VRF3 := (-A3/S3*EXP(-R[3]/S3)+6*C3/power(R[3],7)+ALF1/2*sqr(1-(ALF2+ALF3)/power(R[2],3))*(4/power(R[3],5)+(3*sqr(R[2])-sqr(R[3])-3*sqr(R[1]))/power(R[1],3)/power(R[3],4)))/R[3];
...
procedure tform5.Spectr;
var r,rl,rh,rmin: real;
begin
...
begin
  ...
  with inifile do begin
    a1:=readfloat('Atom','a1',796.0);
    a2:=readfloat('Atom','a2',127.5);
    a3:=readfloat('Atom','a3',62.84);
    p1:=readfloat('Atom','p1',0.5281);
    p2:=readfloat('Atom','p2',0.7073);
    p3:=readfloat('Atom','p3',0.723);
    alf1:=readfloat('Atom','al1',16.8);
    alf2:=readfloat('Atom','al2',16.5);
    alf3:=readfloat('Atom','al3',32.46);
    c1:=readfloat('Atom','c1',247.1);
    c2:=readfloat('Atom','c2',87.36);
    c3:=readfloat('Atom','c3',317.0);

    m1:=readfloat('Atom','m1',83.3);
