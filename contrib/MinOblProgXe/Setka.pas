  ...
  if Radiobutton1.Checked = true then begin
    ...
    if tetmin >= 90 then begin
      ...
      if dtet <= 5 then begin
        tetini := tetmin - 2*dtet; tetend := 180;
      end;
    end;
...
begin
  ...
  if form1.atom = 'Hg' then begin
    if form3.NLO <> 1 then begin
      writeln(tf,
              step, '    ',
              floattostrf(t.eini, fffixed, 7, 3), '     ',
...
begin
  ...
  bend := strtofloat(bend0);
  tetend := strtofloat(tetend0);
  fiend := strtofloat(fiend0);
  bstep := strtofloat(bstep0);
  tetstep := strtofloat(tetstep0);
  fistep := strtofloat(fistep0);
  bini1 := strtofloat(bini0);
  tetini1 := strtofloat(tetini0);
...
begin
  if Application.MessageBox('бНГНАМНБХРЭ ПЮЯВЕР ОН ЯЕРЙЕ, ЙНРНПШИ АШК ОПЕПБЮМ?','йНМРПНКЭ', MB_OKCANCEL) <> IDOK then exit;
  timer1.Enabled: = true;
  continue2: = true;
  continue;
end;
...
begin
  ...
  if timesum<60 then begin
    Label5.Caption := 'оПНЬКН: ' + floattostr(timesum) + ' Я.' + '  пЮЯЯВХРЮМН: ' + floattostr(step) + ' ЬЮЦНБ';
  end;
  if (timesum >= 60) and (timesum < 3600) then begin
    LongTimeFormat := 'nn/ss';
    Label5.Caption := 'оПНЬКН: '+ timetostr((timesum)) + '  пЮЯЯВХРЮМН: ' + floattostr(step) + ' ЬЮЦНБ';
  end;
  Label6.Caption := 'нЯРЮКНЯЭ: ' + timetostr((osttime)) + ' Я.' + '  нЯРЮКНЯЭ ПЮЯЯВХРЮРЭ: ' + floattostr(oststep) + ' ЬЮЦНБ';
end;
