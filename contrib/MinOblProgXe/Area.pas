...
begin
  ...
  if c.rekomb = false then begin
    ...
    for i:=1 to round(180 / step1) do begin
      c.tet := c.tet + step1;
      c.fi := -step1;
      for j := 1 to round(360 / step1) do begin
        c.fi := c.fi + step1;
        t := c;
        senddata;
        c.emolec := form1.t.emolec;
...
begin
  ...
  for i:=0 to k - 1 do begin
    ...
    c1 := false;
    c2 := false;
    fname := form1.atom + form1.edit1.Text + form1.edit2.Text +
             'pointpath' + inttostr(i) + {inttostr(form1.number) + }'.txt';
    assignfile(tf, fname);
    if fileexists(fname) then begin
      append(tf);
    end else begin
    ...
    repeat
      ...
      if form1.atom = 'Hg' then begin
        if form3.NLO <> 1 then begin
          if t.molec = true then begin
            writeln(tf,
                    floattostrf(pointt.eini, fffixed, 7, 3), '   ',
   ...
   `  end;
    ...
    until ((ab = 1) and ((pointt.rekomb = false) or ((pointt.tet = pointprev.tet) and (pointt.fi = pointprev.fi) and (pointt.bet = pointprev.bet)))) or ((ab = -1) and (pointt.rekomb = true));
    closefile(tf);
    ...
    if (ab = 1) and (c2 = false) then begin
      if pointprev.rekomb = false then begin
        if pointprev.molec = true then begin
          writeln(tf,
                  floattostrf(pointprev.eini, fffixed, 7, 3), '   ',
